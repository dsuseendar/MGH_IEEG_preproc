...
utils_folder = fullfile(pwd, 'langloc_utils');
...
edfinfo = edfinfo([PATH_EDF filesep edfname]);
...
channels_table = create_channels_table_bids(edfinfo, PATH_ANNOT, SUBJECT, SESSION);
...
time2save = trialTimingOnset(1)-15*sampling_frequency:trialTimingOnset(end)+15*sampling_frequency; % saving just the necessary folder
timeStart = time2save(1);
natusTimingOnset = (trialTimingOnset-timeStart)./sampling_frequency(1);
...
audioDurNatus = natusAudioEnd-natusTimingOnset;
audioDurBeh = behTimingOffset-behTimingOnset;

isiNatus = [diff(natusTimingOnset(1:40)); diff(natusTimingOnset(41:80)); diff(natusTimingOnset(81:120))];
...
events_table.trial_onset_natus = natusTimingOnset - 0.2;
events_table.audio_onset_natus = natusTimingOnset;
...
obj.condition = (obj.for_preproc.event_table.final_condition);
obj.session = (obj.for_preproc.event_table.final_list);
...
generateExperimentReport(obj, 'EM1289-langloc')
